...
#endif
#define FMI
#define MODEL_IDENTIFIER         OpdrachtDeel1Amesim_
#define MODEL_IDENTIFIER_STRING "OpdrachtDeel1Amesim_"
#endif
...
#define NB_REAL_PARAMS        16
#define NB_INTEGER_PARAMS     4
#define NB_TEXT_PARAMS        0
#define NB_FIXED_VAR          3
#define NB_STATE_VAR          4
...
static ParamInfo ModelParamInfo[ NB_PARAMS ] = {
   ...
   {FixedVar,         Expression,    14,    14, FIXED_CATEGORY,         1},
   {FixedVar,         Expression,    15,    15, FIXED_CATEGORY,         1},
   {FixedVar,         Expression,    16,    16, FIXED_CATEGORY,         1},
   {RealParam,        Expression,     6,    -1, PARAMETER_CATEGORY,     2},
   {RealParam,        Expression,     7,    -1, PARAMETER_CATEGORY,     2},
   ...
   {RealParam,        Expression,    12,    -1, PARAMETER_CATEGORY,     3},
   {RealParam,        Expression,    13,    -1, PARAMETER_CATEGORY,     3},
   {StateVar,         Expression,     0,     0, STATE_CATEGORY,         3},
   {StateVar,         Expression,     1,     1, STATE_CATEGORY,         3},
   {RealParam,        Expression,    14,    -1, PARAMETER_CATEGORY,     4},
   {RealParam,        Expression,    15,    -1, PARAMETER_CATEGORY,     4},
   {StateVar,         Expression,     2,     4, STATE_CATEGORY,         4},
   {StateVar,         Expression,     3,     5, STATE_CATEGORY,         4}
};
...
static double RS2[ 2 ];
static int IS2[ 4 ];
static double *RP2 = &RealParamArray[ 6 ];
static int *IP2 = &IntegerParamArray[ 2 ];
static double RS3[ 1 ];
static int IS3[ 3 ];
static double *RP3 = &RealParamArray[ 12 ];
static double RS4[ 1 ];
static int IS4[ 3 ];
static double *RP4 = &RealParamArray[ 14 ];
...
*/
#ifdef AMERT
double IL_OpdrachtDeel1Amesim_step_ratio=0;
#endif
...
#define v  static_v
#include "OpdrachtDeel1Amesim_.export.h"
#include "OpdrachtDeel1Amesim_.sim.h"
#undef v
#endif
...
DLLEXPORTDEF_OR_STATIC int AMELoadParameters() {
   /* Read OpdrachtDeel1Amesim_.data to pick up submodel parameters */
   AMESIMSYSTEM *amesys;
   ...
   {
      ConstructFileNames(amesys, "OpdrachtDeel1Amesim_", NULL);
      datafilename = GetDataFileName(amesys);
   }
...
#if defined(AMERT) || defined(FMI)
#include "OpdrachtDeel1Amesim_.ssf.h"
...
#include "OpdrachtDeel1Amesim_.subdata.h"
#endif
...
#if defined(AMERT) || defined(FMI)
#include "OpdrachtDeel1Amesim_.globalparams.h"
#endif
...
#if defined(AMERT) || defined(FMI)
   int lineptr=0;
#include "OpdrachtDeel1Amesim_.data.h"
#endif
...
static void Input(AMESIMSYSTEM *amesys, double *y)
{
   ...
   /* Read OpdrachtDeel1Amesim_.sim to pick up simulation parameters */
...
static void Initialize(AMESIMSYSTEM *amesys, double *y)
{
   ...
   n = 1;
   v001in_(&n, &v[14], &v[15], &v[16]);

   n = 1;
   sd0000ain_(&n, RP0, IP0, RS0, IS0, NULL, NULL, NULL, NULL);

   n = 1;
   mas001in_(&n, RP4, RS4, IS4, &y[2], &y[3], NULL, NULL, NULL
      , NULL);
   ...
   n = 1;
   mas002in_(&n, RP3, RS3, IS3, &y[0], &y[1], NULL, NULL, NULL
      , NULL);
...
static void localFunctionEval(AMESIMSYSTEM *amesys, double *dot, double *y, double t, int *flag)
{
   ...
   *oldflag = *newflag = sflag;
   ...
   v[17] = -v[0] /* Duplicate variable. */;
   AME_POST_SUBMODCALL_NO_DISCON(flag);

   v[18] = -v[1] /* Duplicate variable. */;
   AME_POST_SUBMODCALL_NO_DISCON(flag);

   n = 1;
   *oldflag = *newflag = sflag;
   mas001_(&n, &v[4], &dot[2], &v[5], &dot[3], &v[6], &v[7], NULL
      , NULL, &v[28], NULL, NULL, &v[29], NULL, NULL, NULL, NULL
      , RP4, RS4, IS4);
   AME_POST_SUBMODCALL_WITH_DISCON(flag,&sflag,&oflag,&panic,"MAS001",1);

   n = 2;
   *oldflag = *newflag = sflag;
   sd0000a_(&n, &v[13], &v[14], &v[15], &v[17], &v[18], &v[21]
      , &v[22], &v[23], NULL, NULL, &v[24], NULL, NULL, NULL, NULL
      , &v[25], NULL, NULL, RP2, IP2, RS2, IS2);
   AME_POST_SUBMODCALL_WITH_DISCON(flag,&sflag,&oflag,&panic,"SD0000A",2);

   v[20] = v[13] /* Duplicate variable. */;
   AME_POST_SUBMODCALL_NO_DISCON(flag);

   n = 1;
   *oldflag = *newflag = sflag;
   mas002_(&n, &v[0], &dot[0], &v[1], &dot[1], &v[2], &v[3], &v[20]
      , NULL, NULL, &v[26], NULL, NULL, NULL, NULL, &v[27], NULL
      , NULL, RP3, RS3, IS3);
   AME_POST_SUBMODCALL_WITH_DISCON(flag,&sflag,&oflag,&panic,"MAS002",1);

   v[19] = -v[2] /* Duplicate variable. */;
   AME_POST_SUBMODCALL_NO_DISCON(flag);
   ...
   switch (col)
   {
      case 0:
         ...
         mas001_(&n, &v[4], &dot[2], &v[5], &dot[3], &v[6], &v[7]
            , NULL, NULL, &v[28], NULL, NULL, &v[29], NULL, NULL
            , NULL, NULL, RP4, RS4, IS4);
         n = 2;
         sd0000a_(&n, &v[13], &v[14], &v[15], &v[17], &v[18], &v[21]
            , &v[22], &v[23], NULL, NULL, &v[24], NULL, NULL, NULL
            , NULL, &v[25], NULL, NULL, RP2, IP2, RS2, IS2);
         v[20] = v[13] /* Duplicate variable. */;
         n = 1;
         mas002_(&n, &v[0], &dot[0], &v[1], &dot[1], &v[2], &v[3]
            , &v[20], NULL, NULL, &v[26], NULL, NULL, NULL, NULL
            , &v[27], NULL, NULL, RP3, RS3, IS3);
      break;
      case 1:
         v[1] = y[col];
         n = 1;
...
static void AMEDoAFixedStep(double time)
{
   ...
   {
      double localstepratio=floor(IL_OpdrachtDeel1Amesim_step_ratio);
      if (localstepratio >= 1) 
      {
...
DLLEXPORTDEF_OR_STATIC int AMEInitializeConditions(double time, int numOutputs, double *outputARG)
{
   ...
   {
   static int num_fixed = 3;
   static int FIXED[3] = {14, 15, 16};
   ...
   {
#include "OpdrachtDeel1Amesim_.lock.h"
...
#define SYSNME OpdrachtDeel1Amesim_
...
#if defined(AMERT)
#include "OpdrachtDeel1Amesim_.morphee_sigmap.h"
#endif
#if defined(AMERT)
#include "OpdrachtDeel1Amesim_.morphee_parmap.h"
#endif
